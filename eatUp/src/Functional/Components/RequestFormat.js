...
const RequestFormat = ({ requestFrom, updateFriends }) => {
  ...
  const userFriendNetwork = firebase
    .firestore()
    .collection("FriendNetwork")
    .doc(currentUsername);
  const requesterFriendNetwork = firebase
    .firestore()
    .collection("FriendNetwork")
    .doc(requestFrom);
  ...
  const onNotAcceptPressed = (requestFrom) => {
    try {
      userFriendNetwork.update({
        friendRequests: firebase.firestore.FieldValue.arrayRemove(requestFrom),
      });
      requesterFriendNetwork.update({
  ...
  const onAcceptPressed = (requestFrom) => {
    try {
      ...
      userFriendNetwork.update({
        friends: firebase.firestore.FieldValue.arrayUnion(requestFrom),
      });
      requesterFriendNetwork.update({
        ...
        <View style={styles.friendInfoContainer}>
          ...
          <View style={styles.friendInfoText}>
            <TouchableOpacity
              style={styles.button}
              onPress={() =>
                alert("Become friends to view each other's profile!")
              }
            >
